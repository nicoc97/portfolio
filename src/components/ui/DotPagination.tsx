...
export const DotPagination: React.FC<DotPaginationProps> = ({
  ...
  return (
    <nav 
      className={`fixed right-8 top-1/2 -translate-y-1/2 z-50 hidden lg:flex flex-col gap-4 transition-opacity duration-300 ${
    ...
    >
      {sections.map((section) => {
        ...
        return (
          ...
          >
            {/* Tooltip */}
            <div className={`absolute right-8 top-1/2 -translate-y-1/2 px-3 py-1 bg-primary-bg/90 backdrop-blur-sm border border-white/20 rounded-lg font-tech text-sm text-text-primary whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none ${
